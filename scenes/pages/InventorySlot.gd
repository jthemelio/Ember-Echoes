...
func set_item(data: ItemData):
	...
	# 2. Update Icon with Path Checking
	var path = "res://assets/icons/" + data.item_id + ".png"
	if FileAccess.file_exists(path):
		icon.texture = load(path)
	else:
		# Fallback to a generic icon if the specific one is missing
		var fallback = "res://assets/icons/placeholder_" + data.item_type.to_lower() + ".png"
		icon.texture = load(fallback) if FileAccess.file_exists(fallback) else null
